...
def visualize_bev(data, bev, oritraion, vehicle_positions, camera_3d_boxes_pred, camera_3d_boxes_gt, lanes_3d, ego_position, camera_params, camera_names, resolution=0.1, x_range=(-40, 40), y_range=(-40, 40), save_path='bev_visualization.png'):
    ...
    # 将 orientation 转换为弧度
    orientation_rad = np.radians(oritraion)
    ...
    for lane in lanes_3d:
        ...
        for point in lane:
            # 计算点相对于自车的偏移
            dx = point[0] - ego_position[0]
            dy = point[1] - ego_position[1]
            
            # 根据车辆朝向旋转坐标
            rotated_x, rotated_y = rotate_point(dx, dy, -orientation_rad)
            
            # 转换到图像坐标系
            x = rotated_x / resolution + x_size // 2
            y = rotated_y / resolution + y_size // 2
    ...
    for camera_r3dbbx, boxes in camera_3d_boxes_pred.items():
        ...
        for box in boxes:
            ...
            for i in range(8):
                # 计算相对于自车的偏移
                dx = x_coords[i] - ego_position[0]
                dy = y_coords[i] - ego_position[1]
                
                # 根据车辆朝向旋转坐标
                rotated_x, rotated_y = rotate_point(dx, dy, -orientation_rad)
                
                # 转换到图像坐标系
                points_bev[i, 0] = rotated_x / resolution + x_size // 2
                points_bev[i, 1] = rotated_y / resolution + y_size // 2
...
def main():
    fold = "/home/ubuntu/lcj/wan_road/camera"
    ...
    for scene_file in tqdm(scene_files, desc="处理场景"):
        ...
        visualize_bev(data, colored_bev, oritraion, vehicle_positions, camera_3d_boxes_pred, camera_3d_boxes_gt, lanes_3d, ego_position, camera_params, camera_names, save_path=save_path)
